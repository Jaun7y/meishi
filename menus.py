...
def main_menu(con, root_console, background_image, screen_width, screen_height, colours):
    background_image.blit_2x(root_console, 0, 0)

    title = "TOMBS OF THE ANCIENT KINGS"
    centre = (screen_width - len(title)) // 2
    root_console.draw_str(centre, screen_height // 2 - 4, title, bg=None, fg=colours.get('light_pink'))
...
def level_up_menu(con, root, header, player, menu_width, screen_width, screen_height):
    options = ['Constitution (+20HP from {0})'.format(player.fighter.max_hp),
